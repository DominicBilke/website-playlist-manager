...
<input type="date" id="date_from" name="date_from" required <?php if(!isset($_GET['Reset'])) echo 'value="'.(isset($_GET['date_from']) ? $_GET['date_from'] : '').'"'; ?>/>
<input type="submit" value="Reset" name="Reset"/>
<label for="date_to"> to Date: </label>
<input type="date" id="date_to" name="date_to" required <?php if(!isset($_GET['Reset'])) echo 'value="'.(isset($_GET['date_to']) ? $_GET['date_to'] : '').'"'; ?>/>
<input type="submit" value="Set Filter" name="Filter"/>
</form>
<img src="graph.php?id=<?php echo $_GET['id']; ?>&<?php echo (isset($_GET['Reset']) ? 'Reset=1&' : '').(isset($_GET['Filter']) ? 'Filter=1&' : '').'date_from='.(isset($_GET['date_from']) ? $_GET['date_from'] : '').'&'.'date_to='.(isset($_GET['date_to']) ? $_GET['date_to'] : '' ); ?>" style="width:100%;">
<table>
<tr>
	<th>Week</th>
	<th>Month</th>
...
<p align="center">(Values less than 5 min are not displayed!)<br>(The last 90 days are covered!)</p>
</body>
</html>