...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        SET apple_music_minutes = apple_music_minutes + 1,
    ...
    if($stmt->rowCount() == 0) {
        $stmt = $pdo->prepare("
            INSERT INTO user_statistics (user_id, apple_music_minutes, total_minutes, last_updated)
            VALUES (?, 1, 1, NOW())
        ");
        $stmt->execute([$user_id]);
    }
    
    echo json_encode(['success' => true, 'message' => 'Apple Music statistics logged']);
    
} catch(PDOException $e) {
    error_log("Apple Music logging error: " . $e->getMessage());
